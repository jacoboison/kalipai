...
class SaleOrderInherit(models.Model):
    ...
    def action_confirm(self):
        ...
        if self.env.user.has_group('bi_inter_company_transfer.group_ict_manager_access') and setting_id.allow_auto_intercompany:
                    ...
                    for picking in self.picking_ids:
                        ...
                        for move in picking.move_ids:
                            if move.account_move_id:
                                move.account_move_id.write({'partner_id':move.partner_id.id})
            ...
            if self.client_order_ref == False:
                if company_partner_id.id:
                    if self.env.context.get('stop_po') and self.env.context.get('stop_po') == True :
                        pass
                    else :
    ...
    def _create_po_from_so(self , company):
        ...
        if setting_id.create_invoice:
            if setting_id.create_invoice ==True:
                ...
                vals = {'partner_id': po_id.partner_id.id,
                ...
                bill_id = invoice_object.with_context(create_bill = True).sudo().with_company(company).create(vals)
    ...
    def get_po_line_data(self, po_id,company ,line):
        ...
        quantity = line.product_uom_id._compute_quantity(line.product_uom_qty, line.product_uom_id) or line.product_uom_qty
        price = line.product_uom_id._compute_price(price, line.product_uom_id) or price
        return {
            'name': line.name,
